...
public class PainterPartitionProblem {
	...
	public int paint(int A, int B, ArrayList<Integer> C) {
		...
		while (low < high) {
			...
			long totalPainters = getRequiredPainters(C, mid);
			if (totalPainters <= A) {
				high = mid;
			} else
				low = mid + 1;
		...
		long ans = ((low % 10000003) * (B % 10000003)) % 10000003;
		return (int) ans;

	}

	public long getRequiredPainters(ArrayList<Integer> C, long mid) {
